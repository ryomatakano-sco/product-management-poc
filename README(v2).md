...
## 外部コードレビュー対応（~50件、2026-07-14）

各指摘をコードで検証し、実バグを修正。誤り・仕様通りのものは据え置き理由を明記。

**🔴 Critical — 修正**
- `X-Store-Id` 匿名フォールバックを**ループバック限定**に（`deps.get_store_id` + `dev_fallback_allowed`）。リモートの無セッション要求は 401 → dev.py 全体・管理ルートの匿名到達を一括で封鎖（指摘3件を一箇所で解消）。
- `/dev/*`（.env をディスク書換・DBホスト/ユーザー露出）をルーター依存で**ローカル限定**（非ループバックは 404）。
- `POST /inventory/transfer` — 認証なしは上記フォールバック封鎖でカバー（ログイン必須化）。
- `POST /settings/ai/test` を `ensure_admin` に（匿名の OpenAI クォータ浪費を防止／検証: staff 403）。
- PO 入荷の負数 — スキーマ `gt=0` で 422 に加え、エンドポイントでも 400 ガード（検証済み）。
- 部分入荷 PO のキャンセルが在庫を戻さない → 入荷済み分を `apply_stock_delta` で逆仕訳し correction 監査行を記録（検証: 30→34→30）。
- バリアント作成で `on_hand` 直書き → **拠点ミラー未反映のドリフト**。stock サービス経由に変更（検証: total==mirror）。
- バリアント作成で `is_default=true` が既定を降格しない → 既存デフォルトを降格（検証: デフォルトは常に1件）。

**🟠 データ整合 — 修正**
- 承認の二重適用レース → `_get_pending` を `SELECT … FOR UPDATE` に。
- 明細ゼロの PO 送信を 400 でブロック。
- 拠点削除の生 500 → 参照（調整・拠点在庫）を数えてソフト削除、FK 例外も握って 500 回避。
- ダッシュボードの「要対応 top5」が**任意の50件サンプリング**だった → SQL で全商品を available 昇順ランキング。

**🟠🟡 フロント — 修正**
- Support の `formatJpDateTime` 未定義クラッシュ → theme.js にグローバル追加（オフセット/naive 両対応）。
- Login が FastAPI 422 の**配列 detail をそのまま描画**してクラッシュ → 共通 `window.errText()` で安全に文字列化（Categories 保存エラー・再発注失敗にも適用）。
- ダッシュボード AI サマリーの null ガード追加（API エラー時に白画面化しない）。
- 商品一覧・発注書一覧の 100 件上限 → API 上限を 500 に引き上げ + フェッチを 200 に。Vendors も 200（`listVendors` がパラメータ無視だったのを修正）。
- `Number(x) || 10` が入力値 0 を 10 に化かす（Branches しきい値/税率）→ 空欄判定に変更。
- 在庫「在庫切れ」タイルにハンドラ追加、フィルタクリアが拠点フィルタを無視 → 対応。
- PO CSV エクスポートが検索語を無視 → `q` を渡す（バックエンド export.csv も `q` 受付）。
- UnderConstruction「通知を受け取る」に onClick（トースト）。
- 税率エディタが保存後に再同期しない（`rows===null` 一回きり）→ サーバー値の署名で毎回同期。
- 商品詳細の再発注 URL 失敗を握りつぶし → エラートースト。variants.reduce が欠損値で NaN → `Number()||0`。
- 手動販売の `sold_at` が**ブラウザTZ依存** → `+09:00` を明示付与（JST 固定）。
- AdminShell `relTime` が `+09:00` 付き文字列に誤って Z を足し NaN → オフセット検出。
- i18n: 最終更新ラベル・タイムスタンプ locale をロケール対応化（バッチC/Dの他リークは前回スイープで対応済み）。
...
## ロジック総点検 — 8件の修正 + 未使用要素マーキング（2026-07-15）

全ページの機能・ボタン・KPI・リスト・サイドバーを論理レビューし、8件を修正。

| # | 修正 | 内容 |
|---|---|---|
| 1 | **在庫低下の定義を統一** | ダッシュボードKPI・在庫ページ・商品一覧が「available ≤ 10」ハードコードで、通知/自動発注の**バリアント毎しきい値**と食い違っていた（しきい値20の商品がベルは鳴るのに画面は正常表示）。全箇所 `low_stock_threshold`（既定10）基準に統一。 |
| 2 | **商品一覧を新しい順に** | `ORDER BY id ASC` で新規登録商品が最終ページに落ちていた → 降順に。 |
| 3 | **カテゴリの親子付け替え UI** | ツリー表示はあるのにフォームに親選択がなかった → 「親カテゴリ」ドロップダウン追加（自分と子孫は選択不可）。バックエンドに自己参照・循環ガード（400）。検証: 設定200 / 循環400 / 自己400。 |
| 4 | **サイドバー: カテゴリを マスタ へ** | カテゴリは初期設定後ほぼ触らないマスタデータ — メイン（毎日使う画面）から マスタ（カテゴリ・仕入先・院/店舗）へ移動。チュートリアルの説明文も更新。 |
| 5 | **設定の並び替え + 統合にモック表示** | ユーザー管理を2番目へ（監査ログも同居）、外観・言語は後方、統合は最後尾 + 「モック表示・PoCでは動作しません」バナー。 |
| 6 | **最近の活動に実名表示** | ダッシュボードの活動フィードが常に「システム」→ mig 016 の `created_by` を表示（検証: 山田 花子）。 |
| 7 | **PO KPI: 一部入荷 → 未入荷金額** | 一部入荷件数は直下のチップと重複 → 発注済み未入荷の金額 Σ(未入荷数×単価) に置換（`/purchase-orders/summary` に `outstanding_value` 追加）。 |
| 8 | **拠点の営業時間を編集可能に** | 詳細で表示されるのにフォームで編集不可だった → 曜日別テキスト入力（`09:00-13:00 / 14:30-18:00`、空欄=休診）で JSON 形へパース。形式エラーは保存前に警告。検証: ラウンドトリップOK。曜日ラベルはロケール対応（dict の1文字キーは 日→Sun 事故クラスのため回避）。 |

**未使用要素スイープ** — コードに `[UNUSED-SWEEP 2026-07-15]` コメントでマーキング（削除はしていない・要判断）:
- frontend/lib/api.js: `listStores` / `createVariant` / `getCategory` / `deleteVendor` / `deleteBranch`（UI に削除ボタンが無いため未使用 — ボタンを付けるか削除するか要判断）+ `window.PLX_API` エイリアス
- frontend/lib/jan.js: `plxNormalizeJan` / `plxValidateCheckDigit` の window バインディング（内部関数としては使用中）
...
## ウェアハウスマネージャー観点の4修正（2026-07-15）

| # | 問題 | 対応 |
|---|---|---|
| 1 | **使用期限のデータ断絶** — アラートは手入力の `products.expiry_date` を読む一方、入荷はロット毎の実期限を記録し、両者は同期しない（新ロット入荷でアラートが古いまま） | `services/expiry.py`: **有効期限 = 有効ロット（在庫>0）の最も早い期限、なければ手入力値**。ダッシュボードKPI・要対応・在庫一覧・商品一覧の期限フィルタ/バッジ・日次サマリー通知の全てがこれを使用。検証: PO-12 でロット捕捉後、商品7の表示期限が最古有効ロットに一致（FEFO と整合）。 |
| 2 | **引当列が常に0** — 予約機能が存在しないのに列が予約を示唆 | 在庫テーブルから 引当 列を削除（利用可能は従来通り net 値）。商品詳細の 引当中/使用不可 行は **非ゼロ時のみ**表示。棚卸しCSVの列構成は不変（取込の列番号互換を維持）。手動調整での引当操作は引き続き可能。 |
| 3 | **入荷予定/遅延が見えない** — ETA は記録するのにどこにも出ない | ダッシュボード回転KPIに「入荷予定（7日以内）」タイル（遅延があれば赤 + 遅延N件）。PO一覧の納品予定日に **遅延ピル**（ETA超過かつ未入荷完了）。summary に `incoming_pos_7d` / `overdue_pos`。検証: 入荷予定2件・遅延0件。 |
| 4 | **滞留在庫が見えない** — 在庫低下の逆（売れずに眠る在庫） | `GET /dashboard/slow-movers?days=60`: 在庫あり・直近N日販売なしの商品（返品は動きに数えない、未販売が先頭）。ダッシュボードに「滞留在庫」カード（在庫数・在庫金額・最終販売、クリックで商品へ）。検証: 6件検出。 |

おまけ: サイドバーのワークスペース行が固定「本院 / 管理者」だった → ログインユーザーのロール表示に。
